...
`babelfish install` times out a hung `git clone` after 120 seconds. Pass `--clone-timeout-ms <ms>` or set `OPENCLAW_BABELFISH_CLONE_TIMEOUT_MS` when a healthy clone needs longer.
